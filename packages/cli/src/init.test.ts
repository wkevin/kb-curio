...
describe('kb-curio init', () => {
  ...
  it('--vendor copies the framework build into <target>/vendor/ and rewrites the dep to file:', async () => {
    // Vendor mode is the source-decoupled middle ground: not a symlink to
    // the framework checkout (link:), not a published version (^x.y.z), but
    // a real snapshot of packages/core's `files` field sitting inside the
    // scaffolded project.
    const target = path.join(tmpDir, 'vendored');

    await init({ dir: target, install: false, git: false, vendor: true });

    // package.json rewritten to file:./vendor/@kb-curio/core
    const pkg = JSON.parse(fs.readFileSync(path.join(target, 'package.json'), 'utf8'));
    expect(pkg.dependencies['@kb-curio/core']).toBe('file:./vendor/@kb-curio/core');

    // vendor/ directory exists and contains the published artefacts
    const vendorDir = path.join(target, 'vendor', '@kb-curio', 'core');
    expect(fs.existsSync(vendorDir)).toBe(true);
    expect(fs.existsSync(path.join(vendorDir, 'package.json'))).toBe(true);
    // dist/ is built by `pnpm --filter @kb-curio/core build`; if it's
    // missing the vendor step silently shipped an empty package.
    expect(fs.existsSync(path.join(vendorDir, 'dist'))).toBe(true);

    // .gitignore picks up `vendor/` so the snapshot isn't committed
    const gi = fs.readFileSync(path.join(target, '.gitignore'), 'utf8');
    expect(gi.split(/\r?\n/).some((l) => l.trim() === 'vendor/')).toBe(true);

    // The real framework checkout is NOT polluted (no workspace registration).
    const repoRoot = path.resolve(__dirname, '..', '..', '..');
    const wsPath = path.join(repoRoot, 'pnpm-workspace.yaml');
  ...
  it('--vendor takes priority over auto-detected local mode', async () => {
    // Same setup as the link: sibling test: the CLI-source detector finds
    // the real monorepo. With --vendor, the result must be `file:` even
    // though a monorepo is reachable (which would normally produce
    // workspace:* / link:).
    const target = path.join(tmpDir, 'vendor-overrides-local');
    await init({ dir: target, install: false, git: false, vendor: true });
